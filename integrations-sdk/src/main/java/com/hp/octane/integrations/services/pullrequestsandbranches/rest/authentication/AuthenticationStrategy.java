...
public abstract class AuthenticationStrategy {
    ...
    /**
     * Check if need to do authentication before request
     *
     * @return true if need to call authentication before sending other requests
     */
    ...
    public void onResponse(HttpResponse httpResponse, HttpClientContext context) {
        if (isManageCookies()) {
            context.getCookieStore().getCookies().forEach(c -> cookies.put(c.getName(), c));
        }
    }

    public void onCreateContext(HttpClientContext context) {
        if (isManageCookies()) {
            cookies.values().forEach(c -> context.getCookieStore().addCookie(c));
        }
    }
